...
async function main() {
    ...
    for (const type of ['rss', 'podcast']) {
        ...
        while (!feedsSynced) {
            try {
                const feedCursor = model.feed.collection.find({
                    _id: { $gte: lastFeedId }
                }).sort({
                    _id: 1
                }).batchSize(32);
                while (await feedCursor.hasNext()) {
                    ...
                    const allowedLanguage = [null, undefined, '', 'eng'].includes(feed.language);
                    if (!allowedLanguage) {
                        bar.tick();
                        continue;
                    }
                    ...
                    while (!articlesSynced) {
                        try {
                            const cursor = model.content.collection.find({
                                _id: { $gte: lastId },
                                [type]: feed._id,
                            }).sort({
                                publicationDate: 1,
                                _id: 1
                            }).limit(1000).batchSize(1000);
                            const articleCount = await model.content.countDocuments({ [type]: feed._id });
                            let upserts = [];
                            let content = [];
                            let currentSize = 0;
                            ...
                            while (await cursor.hasNext()) {
                                ...
                                if (batchIsFull || batchTooBig) {
                                    upserts.push(upsertCollections(contentModelName, content));
                                    content = [];
                                    currentSize = 0;
                                }
