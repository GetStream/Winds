...
async function main() {
    ...
    let publicationIDs = group.map(c => {return c.publicationID})
    let schema = (group[0].type) == 'rss' ? RSS : Podcast
    let followerCount = group[0].followers
    logger.info(`Starting update of ${publicationIDs.length} publications of type ${group[0].type} to count ${followerCount}`)
    let result = await schema.update(
  		{ $and: [
        {_id: {$in: publicationIDs}}, {followerCount: {$ne: followerCount}}
      ] },
  		{
