...
async function main() {
    ...
    const urlToScore = posts.reduce((map, post) => map.set(post.url, post.score), new Map());
    const commentUrlToScore = posts.reduce((map, post) => map.set(commentUrl(post.id), post.score), new Map());
    ...
    for (const match of urlMatch) {
        batch.find({ _id: match._id }).updateOne({$set: { 'socialScore.hackernews': urlToScore.get(match.url) }});
    }
    for (const match of commentUrlMatch) {
        batch.find({ _id: match._id }).updateOne({$set: { 'socialScore.hackernews': commentUrlToScore.get(match.commentUrl) }});
    }
    await batch.execute();
}
