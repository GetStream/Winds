...
async function mergeFollows(lhsID, rhsID, type) {
	...
	const update = (rhsFollows || []).filter(f => f.user && !lhsUsers.has(String(f.user._id)));
	const remove = (rhsFollows || []).filter(f => f.user && lhsUsers.has(String(f.user._id)));
...
async function mergeArticlesAndPins(lhsID, rhsID, type) {
	...
	const lhsArticlesWithMatchingFingerprints = await model.find({
		rss: lhsID,
	...
	const duplicatePins = removePins.filter(p => lhsPinFingerprints.has(p[field].fingerprint));
	const newPins = removePins.filter(p => !lhsPinFingerprints.has(p[field].fingerprint));

	const pinUpdates = newPins.map(p => {
		const article = lhsArticlesWithMatchingFingerprints.filter(a => a.fingerprint == p[field].fingerprint)[0];
