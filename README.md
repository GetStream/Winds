...
**Winds 2.0 is a easy-to-use, open-source RSS and Podcast app built using React/Redux/Node with a beautiful UI. Not only are the activity streams & personalization in Winds powered by [Stream](https://getstream.io/get_started), it also leverages [Algolia](https://algolia.com?ref=stream) for search, [Amazon Web Services (AWS)](https://aws.amazon.com/) for hosting via ECS, [MongoDB Atlas](http://mbsy.co/mongodb/228644) for the hosted database as a service (DBaaS), and [SendGrid](https://sendgrid.com/) for email deliverability.**
